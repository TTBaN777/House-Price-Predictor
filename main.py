...
def main():
    ...
    # --- 2. 使用者輸入縣市與筆數 ---
    while True:
        target_city = input(f"請輸入想搜集的縣市 (例如: 台北市, 新北市): ").strip()
    ...
    while True:
        try:
            total_count = int(input("需要抓取幾筆資料？(以 10 為單位): "))
            if total_count > 0:
    ...
    if os.path.exists(csv_path):
        os.remove(csv_path)
        print(f"已刪除舊的 CSV 檔：{csv_path}")

    # --- 4. 檔案處理完畢，現在才初始化資料庫連線 ---
    db = Database()

    # --- 5. 開始執行搜集迴圈 ---
    print(f"\n開始搜集 {target_city} 的租屋資料，預計抓取 {total_pages} 頁...")
    ...
    # --- 6. 產出報表 ---
    print(f"\n--- 任務完成 ---")
    print(f"總共抓取並過濾出 {actual_count} 筆資料")
